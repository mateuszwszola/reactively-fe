...
test("User performs full flow", async ({page}) => {
    ...
    await page.getByPlaceholder('Enter post title').fill('How to start working out');

    await page.getByPlaceholder('Enter post content').click();

    await page.getByPlaceholder('Enter post content').fill('Start by building a habit');

    await page.getByRole('button', { name: 'Create Post' }).click();

    await page.getByRole('link', { name: 'ExerciseEveryday HomeWorkout How to start working out test001 0 people liked this' }).getByRole('button').click();

    await page.getByRole('link', { name: 'ExerciseEveryday HomeWorkout How to start working out test001 1 people liked this' }).click();
    ...
    await page.getByRole('link', { name: 'ExerciseEveryday HomeWorkout How to start working out test001 1 people liked this' }).getByRole('link', { name: 'test001' }).click();
